...
export async function layoutGraph<TNode extends Node, TEdge extends Edge>(
  nodes: TNode[],
  edges: TEdge[],
  options: {
...
function getNodeSize(node: Node, key: "width" | "height", fallback: number) {
...
function getProviderHandleY(node: Node, handleId: string): number {
  ...
  return blockStartY + getProviderBlockHeight(data?.providers?.length ?? 0) / 2;
}
